...
class PhotoCommentsNetworkService: PhotoCommentsServiceProtocol {
    ...
    func fetchPhotoComments(photoId: String, page: Int, limit: Int, completionHandler: @escaping (Result<[STComment], OperationError>) -> Void) {
        let operation = GetCommentsOperation(model: GetCommentsOperationModel.Request(photoId: photoId, limit: limit, page: page)) { result in
            switch result {
